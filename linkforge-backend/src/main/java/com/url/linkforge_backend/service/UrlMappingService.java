...
import com.url.linkforge_backend.dtos.ClickEventDTO;
import com.url.linkforge_backend.dtos.UrlMappingDTO;
import com.url.linkforge_backend.models.ClickEvent;
import com.url.linkforge_backend.models.UrlMapping;
import com.url.linkforge_backend.models.User;
...
public class UrlMappingService {
    ...
    public void deleteUrl(String shortUrl, User user) {
        UrlMapping urlToDelete = urlMappingRepository.findByShortUrl(shortUrl);
        if (urlToDelete == null) {
            throw new EntityNotFoundException("URL not found with code: " + shortUrl);
        }
        if (!urlToDelete.getUser().getUsername().equals(user.getUsername())) {
            throw new AccessDeniedException("You do not have permission to delete this link.");
        }
        urlMappingRepository.delete(urlToDelete);
